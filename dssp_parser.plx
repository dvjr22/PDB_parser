...
sub ProcessFiles {
	...
	my $insert = "insert into phi_psi (protein, resSeq, resName, phi, psi, alpha, kappa) values (?, ?, ?, ?, ?, ?, ?) ";
	...
	# Place file names into map to attach full path
		...
		while (<IN>) {
			...
			if ($acknowledged eq $engage) {
				...
				# Insert values
				$statement -> execute($protein, $resSeq, $resName, $phi, $psi, $alpha, $kappa);
